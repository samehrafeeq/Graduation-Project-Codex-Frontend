...
const Register = () => {
  ...
  const text = {
    title: isArabic ? 'انضم إلى مجتمعنا' : 'Join Our Community',
    subtitle: isArabic ? 'اختر نوع الحساب الذي يناسب احتياجاتك لتبدأ رحلتك معنا' : 'Choose the account type that fits your needs and start your journey',
    buyer: isArabic ? 'مشتري' : 'Buyer',
    buyerDesc: isArabic
      ? 'ابحث عن خدمات احترافية، تواصل مع المبدعين، وانجز مشاريعك بسهولة وأمان.'
      : 'Find professional services, connect with top talent, and complete your projects safely.',
    buyerCta: isArabic ? 'سجل كمشتري' : 'Register as Buyer',
    seller: isArabic ? 'بائع خدمات' : 'Seller',
    sellerDesc: isArabic
      ? 'اعرض مهاراتك، قدم خدماتك للآلاف من العملاء، وابدأ في تحقيق دخل مستمر.'
      : 'Showcase your skills, offer services to clients, and start building recurring income.',
    sellerCta: isArabic ? 'سجل كبائع' : 'Register as Seller',
    ...
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden py-12 bg-muted/30">
      ...
      <div className="relative z-10 w-full max-w-4xl px-4 animate-in fade-in zoom-in-95 duration-500">
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="text-3xl md:text-4xl font-extrabold text-foreground mb-4">{text.title}</h1>
          <p className="text-lg text-muted-foreground max-w-lg mx-auto">{text.subtitle}</p>
        </div>

        <div className="grid md:grid-cols-2 gap-6 max-w-3xl mx-auto">
          {/* Buyer Card */}
          <Link to="/register/buyer" className="group">
            <Card className="h-full border-0 shadow-xl shadow-black/5 bg-card hover:bg-card/90 transition-all hover:scale-[1.02] hover:shadow-2xl overflow-hidden relative">
              ...
              <CardContent className="p-8 flex flex-col items-center text-center h-full">
                ...
                <h3 className="text-2xl font-bold text-foreground mb-3 group-hover:text-blue-400 transition-colors">{text.buyer}</h3>
                <p className="text-muted-foreground mb-8 leading-relaxed">
                  {text.buyerDesc}
                </p>
                <Button className="mt-auto w-full bg-blue-600/15 text-blue-700 dark:text-blue-300 hover:bg-blue-600 hover:text-white font-bold h-12 rounded-xl transition-all">
                  {text.buyerCta}
          ...
          <Link to="/register/seller" className="group">
            <Card className="h-full border-0 shadow-xl shadow-black/5 bg-card hover:bg-card/90 transition-all hover:scale-[1.02] hover:shadow-2xl overflow-hidden relative">
              ...
              <CardContent className="p-8 flex flex-col items-center text-center h-full">
                ...
                <h3 className="text-2xl font-bold text-foreground mb-3 group-hover:text-purple-400 transition-colors">{text.seller}</h3>
                <p className="text-muted-foreground mb-8 leading-relaxed">
                  {text.sellerDesc}
                </p>
                <Button className="mt-auto w-full bg-purple-600/15 text-purple-700 dark:text-purple-300 hover:bg-purple-600 hover:text-white font-bold h-12 rounded-xl transition-all">
                  {text.sellerCta}
                  <ArrowLeadingIcon className="mr-2" />
                </Button>
              </CardContent>
